...
协作导航当前四方法试跑已审查并配置：见[协议审查与试跑命令](cooperative_navigation/当前协议审查与四方法试跑.md)。配置在`configs/navigation_trial/`，输出集中到`results/navigation_trial/`。这是单种子工程与学习试跑，不是已收敛或理论保证成立的声明。
