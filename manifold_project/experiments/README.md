...
更新：2026-09-22。跨环境文档的职责和阅读顺序见[研究文档导航](../docs/README.md)。本页负责实现进度与直接入口；环境中的设计稿、可运行代码和已完成结果是三种不同状态。

| 目录 | 环境 | 当前状态 |
|---|---|---|
| [pair_coordination](pair_coordination/README.md) | 可枚举成对协作 | 83次开发训练及三组确认补充实验已完成；新增60次训练、640条方向/180条运输/9600条检查记录 |
| [cooperative_navigation](cooperative_navigation/README.md) | MPE2 合作导航 | 首次同时全覆盖即结束；旧训练产物已清理，仅保留首达诊断及来源，新协议尚未长训练，参数未冻结 |
...
| 合作导航 | 已完成旧PPO首达诊断；新首达协议先训练MAPPO/IPPO与ours/no_checks，各2000万步 | [运行说明](cooperative_navigation/实验运行说明.md)、[当前判断](cooperative_navigation/results/当前实验判断.md)、[结果索引](cooperative_navigation/results/README.md) |
...
第一条只打印计划，不创建结果；需要实际 smoke 时移除 --dry-run 并按需添加 --plot。旧 run_demo.py 已不存在。导航通过run_experiments.py运行，当前须显式传入configs/first_arrival中的配置，完整命令见[导航运行说明](cooperative_navigation/实验运行说明.md)；仓库仍无对应训练入口。
