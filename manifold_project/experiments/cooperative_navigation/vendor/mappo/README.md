...
任务适配集中在[training/author_ppo.py](../../training/author_ppo.py)：共同MPE2场景、采样预算、随机动作采样、终止/采样截断和统一评价。MAPPO Critic读集中状态，IPPO只读本地历史；两者均关闭RNN。当前首达任务将真实变长轨迹打包，内部真终止阻断GAE，批末采样截断保留最终价值；旧持续任务仍可复现。适配边界及保存见[实现说明](../../实现说明.md#完整方法的一轮)。本版本称“基于作者实现、适配统一MPE2任务协议的MAPPO/IPPO”，不等同于复现作者论文原始场景及成绩。
